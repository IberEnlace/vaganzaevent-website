...
import { CalendarPlus, Edit3, LogOut, Plus, Star, Trash2, X } from "lucide-react";
...
export default function AdminClient({ authenticated, initialEvents }: { authenticated: boolean; initialEvents: EventItem[] }) {
  ...
  function edit(item: EventItem) { const { id, ...values } = item; setEditing(id); setForm(values); setModal(true); }
  ...
  return <main className="admin-shell">
    ...
    <section className="admin-main"><header><div><p className="eyebrow">CONTENT MANAGEMENT</p><h1>Events</h1><span>Create, publish and curate Vaganza nights.</span></div><button onClick={() => { setForm(empty); setEditing(null); setModal(true); }}><Plus /> New event</button></header>
    ...
    {modal && <div className="admin-modal"><div className="modal-panel"><header><div><p className="eyebrow">{editing ? "EDIT" : "CREATE"}</p><h2>{editing ? "Edit event" : "New event"}</h2></div><button onClick={() => setModal(false)}><X /></button></header><form onSubmit={save}>
      ...
      <label className="full">Image URL<input required type="url" value={form.image} onChange={(e) => setForm({ ...form, image: e.target.value })} /></label></div>
      ...
      {message && <div className="admin-error">{message}</div>}<div className="form-actions"><button type="button" onClick={() => setModal(false)}>Cancel</button><button type="submit">{editing ? "Save changes" : "Create event"}</button></div>
    </form></div></div>}
  </main>;
}
